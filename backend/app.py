...
def search():
    ...
    companies = db.session.execute(
        text("SELECT symbol, name FROM companies WHERE LOWER(name) LIKE :search OR LOWER(symbol) LIKE :search LIMIT 10"),
        {"search": f"{query}%"}
    ).fetchall()

    suggestions = [{"symbol": row.symbol, "name": row.name} for row in companies]
